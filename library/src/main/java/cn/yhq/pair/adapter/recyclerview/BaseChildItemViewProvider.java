...
public abstract class BaseChildItemViewProvider<T extends PairItem> extends ItemViewProvider1<PairCatalog> {
    ...
    public void onBindViewHolder(ViewHolder viewHolder, int position, PairCatalog entity) {
        PairAdapter adapter = (PairAdapter) this.getAdapter();
        int groupPosition = adapter.getGroupPosition(position);
        int childPosition = adapter.getChildPosition(position);

        PairCatalog groupEntity = adapter.getGroup(groupPosition);
        PairItem childEntity = adapter.getChild(groupPosition, childPosition);
        ...
        if (childPosition == adapter
                .getChildCount(groupPosition) - 1) {
            viewHolder.setVisibility(View.GONE);
        } else {
